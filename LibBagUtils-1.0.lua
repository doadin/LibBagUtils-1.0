...
function lib:IterateBags(which, itemFamily)
    ...
    if not IsClassicWow() and which == "REAGENTBANK" and not IsReagentBankUnlocked() then return function() end, baglist end
...
function lib:Iterate(which, lookingfor)
    if not IsClassicWow() and which == "REAGENTBANK" and not IsReagentBankUnlocked() then return function() end end
...
function lib:Find(where,lookingfor,findLocked)
    if not IsClassicWow() and where == "REAGENTBANK" and not IsReagentBankUnlocked() then return nil end
